...
start_gpfdist()
{
	...
	/usr/local/hawq/bin/gpfdist -d /$DATA_DIR/$DEMO_DIR/data -p $GPFDIST_PORT >> /dev/null 2>&1 < /dev/null &
	gpfdist_pid=$!
...
create_reports_schema()
{
	...
	echo "############################################################################"
	local MY_DIR=$1
	local schema_count=`psql -h localhost -U gpadmin -p 5432 -A -t -c "SELECT count(*) from pg_namespace WHERE nspname = 'reports'"`
	if [ "$schema_count" -eq "0" ]; then
		psql -h localhost -U gpadmin -p 5432 -c "CREATE SCHEMA reports;"
	fi

	local table_count=`psql -h localhost -U gpadmin -p 5432 -A -t -c "SELECT count(*) from pg_namespace n JOIN pg_class c ON c.relnamespace = n.oid  WHERE n.nspname = 'reports' AND c.relname = '$DEMO' AND relstorage = 'x'"`
	if [ "$table_count" -eq "0" ]; then
		psql -h localhost -U gpadmin -p 5432 -c "CREATE EXTERNAL WEB TABLE reports.$DEMO (id int, table_name varchar, duration time) EXECUTE 'cat \"$MY_DIR/rollout.log\"' ON MASTER FORMAT 'TEXT' (DELIMITER '|');"
	fi
	echo ""
}
